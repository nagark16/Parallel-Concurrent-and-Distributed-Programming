...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
        	if(endIndexExclusive - startIndexInclusive <= theshold) {
    ...
    protected static double parManyTaskArraySum(final double[] input,
            final int numTasks) {
        double sum = 0;
        int chuckSize = getChunkSize(numTasks, input.length);
        int low = 0;
        int high = 0;
        ForkJoinPool pool = new ForkJoinPool(numTasks);
        ReciprocalArraySumTask t;
        for(int i = 0; i < numTasks; i++) {
        	low = getChunkStartInclusive(i, numTasks, input.length);
        	high = getChunkEndExclusive(i, numTasks, input.length);
        	t = new ReciprocalArraySumTask(low, high, input);
        	pool.invoke(t);
            sum += t.getValue();
        }
